from airflow.sdk import task, dag, task_group, Param, get_current_context
...
def pic():
    ...
    def tracker(dag_run):
        # fix for Airflow 3
        res = RuntimeTaskInstance.get_task_states(dag_run.dag_id, run_ids=[dag_run.run_id], task_ids=['clean'])

        # format for res: { run_id: {task_id: state} }
        for v in res.values():
            state = v.get('clean')

            # if the state is not `None`, it means that all workers are done
